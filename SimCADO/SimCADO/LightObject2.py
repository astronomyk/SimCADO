...
class Source(object):
    ...
    def apply_optical_train(self, opt_train, **kwargs):
        """
        ...
        for i in range(len(opt_train.lam_bin_edges[:-1])):
            if params["verbose"]: 
                print("Wavelength slice [um]:", opt_train.lam_bin_centers[i])
            ...
            chip_no = params["chip_no"]
            if self.array == None:
                self.array = self.image_in_range(psf, lam_min, lam_max, 
                                                 opt_train.chips[chip_no])
            else: 
                self.array += self.image_in_range(psf, lam_min, lam_max, 
                                                  opt_train.chips[chip_no])
        ...
        # 4.
        if params["ATMO_BG_ON"].lower() == "yes":
