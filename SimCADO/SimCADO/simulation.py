...
def snr(mags, filter="K", exptime=18000, ndit=1, cmds=None):
    """
    ...
    for a list of magnitudes in `mags` in a certain broadband `filter`. A 
    custom UserCommands object can also be used. Note that this runs a basic 
    ...
    cmd["OBS_NDIT"] = ndit
    cmd["INST_FILTER_TC"] = filter
    
    opt = simcado.OpticalTrain(cmd)
    
    sn = []
    for mag in mags:
        src = simcado.source.star(mag)
