...
def get_total_wing_leakage(fluxes, wavelengths, transmission_coeffs=1E-3, 
    ...
    tbl = Table(data=[row_names]+frac_list+[sum_frac], names=["Name"]+reg_names+["Total"])
    
    
    if make_plot:
        for fracs, spt, clr in zip(frac_list, spec_types+["Sky"], "bgr"):
            fracs = list(fracs)
        ...
        plt.xlabel("Wavelength [um]"); plt.ylabel("Extra flux fraction per region")
...
def required_transmission(fluxes, total_flux_increase=1E-2, filter_index=1, 
    ...
    coeff_list = np.array(coeff_list).T.tolist()
    reg_names = get_region_names(wavelengths, filter_index)
    
    if row_names is None:
        row_names = ["Object "+str(i) for i in range(len(fluxes))]
    
    tbl = Table(data=[row_names]+coeff_list, names=["Name"]+reg_names)
